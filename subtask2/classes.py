...
class IndexingAnnotation(Annotation):
    ...
    def __init__(self, file_name=None, root="root", subtask_ann= None):
        self.doc_id = ''
        self.sys_id = ''
        self.root = root
        self.indexes = []
        ...
        if file_name:
            self.sys_id = os.path.basename(os.path.dirname(file_name))
            self.doc_id = os.path.splitext(os.path.basename(file_name))[0]
            if subtask_ann is not None:
                unclear_tag, correct_tag = self.filter_uncleartags(os.path.join(subtask_ann,self.doc_id+".ann"), tagunclear=False)
                unclear_label, correct_label = self.filter_uncleartags(os.path.join(subtask_ann, self.doc_id + ".ann"), True, unclear_tag, correct_tag)
            self.parse_indexes(file_name, unclear_label,correct_label)
            self.file_name = file_name
        else:
            self.doc_id = None
    ...
    def filter_uncleartags(self, file_name=None, tagunclear = False,unclear_tag=None, correct_tag=None):
        unclear = []
        correct = []
        if file_name is not None:
            for row in open(file_name, 'r'):
                ...
                if tagunclear == False:
                    if line.startswith("T"):
                        ...
                        if tag.strip() == "UNCLEAR":
                            unclear.append(line.split("\t")[0].strip())
                        else:
                            correct.append(line.split("\t")[0].strip())
                if tagunclear == True:
                    if line.startswith("#"):
                        label = line.split("\t")[1].split()[1].strip()
                        if label in correct_tag:
                            correct.append(line.split("\t")[2].strip())
                        if label in unclear_tag:
                            unclear.append(line.split("\t")[2].strip())
        return unclear, correct

    def parse_indexes(self, file_name=None, unclear_label=None, correct_label=None):
        if file_name is not None:
            for row in open(file_name, 'r'):
                line = row.strip()
                sys_id = line.split("\t")[0]
                ...
                if sys_id == self.doc_id:
                    if (sys_code not in unclear_label or sys_code in correct_label)and sys_code not in self.indexes:
